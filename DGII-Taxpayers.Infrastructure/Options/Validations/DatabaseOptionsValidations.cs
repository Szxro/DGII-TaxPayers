...
internal class DatabaseOptionsValidations : AbstractValidator<DatabaseOptions>
{
    public DatabaseOptionsValidations()
    {
        ...
        RuleFor(options => options.MaxRetryCount).Must(count => count < int.MinValue).WithMessage("The {PropertyName} cant be zero or lower than zero");
